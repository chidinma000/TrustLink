...
pub enum StorageKey {
    ...
    /// List of endorsements for an attestation.
    Endorsements(String),
    /// Configurable storage limits.
    Limits,
    /// Rate limit configuration.
    RateLimitConfig,
    /// Last issuance timestamp per issuer.
    LastIssuanceTime(Address),
    /// Multi-sig proposal by ID.
    MultisigProposal(String),
    /// Audit log for an attestation.
    AuditLog(String),
    /// Whitelist enabled flag per issuer.
    WhitelistEnabled(Address),
    /// Whitelist entries per issuer.
    Whitelist(Address, Address),
    /// Attestation request by ID.
    Request(String),
    /// Pending request IDs per issuer.
    PendingRequests(Address),
    /// Delegation from delegator to delegate for a claim type.
